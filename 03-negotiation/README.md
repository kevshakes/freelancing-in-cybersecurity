# 03 - Negotiation

Holding your rate and your boundaries once you are in conversation with a client.

| File | Purpose |
|---|---|
| [rate-negotiation-guide.md](./rate-negotiation-guide.md) | Responses to price pushback, what to trade instead of discounting |
| [objection-handling-scripts.md](./objection-handling-scripts.md) | Scripts for common direct-client objections |
| [scope-creep-and-boundaries.md](./scope-creep-and-boundaries.md) | Recognizing and responding to scope creep |

Back to [main README](../README.md).
